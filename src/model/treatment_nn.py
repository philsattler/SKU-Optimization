...
class TreatmentModel:
    def __init__(self, input_dim, output_dim, lr=0.001, patience=10):
        self.model = MultiLabelNN(input_dim=input_dim, output_dim=output_dim)
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, num_epochs=100, batch_size=10):
...
if __name__ == '__main__':
    from sklearn.model_selection import train_test_split
    from src.data.data_pipeline import read_data, treatment_data_processing
    ...
    X = scaler.fit_transform(final_data.drop(columns=['store_id']).values)
    ...
    residuals = treatment_model.residual(X, y)
    print(residuals)